...
class LeNet5:
    ...
    def build_model(self, nof_outputs, print_summary=True):
        # Building the model
        model = Sequential([
            Rescaling(1, input_shape=(self.input_dim, self.input_dim, 1)),
            Conv2D(filters=6, kernel_size=(5, 5), activation='relu'),
            AveragePooling2D(pool_size=(2, 2)),
            Conv2D(filters=16, kernel_size=(5, 5), activation='relu'),
            AveragePooling2D(pool_size=(2, 2)),
            Conv2D(filters=120, kernel_size=(4, 4), activation='relu'),
            Dropout(0.2),
            Flatten(),
            Dense(units=120, activation='relu'),
            Dense(units=nof_outputs, activation='softmax')
        ])
    ...
    def train_shape_classifier(self):
        ...
        self.save_model_plots(history, "Results/CNN/ACC_Loss_Graphs", "shape")
    ...
    def train_type_classifier(self):
        ...
        self.save_model_plots(history, "Results/CNN/ACC_Loss_Graphs", "type")
